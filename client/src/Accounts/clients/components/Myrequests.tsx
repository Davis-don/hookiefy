...
function Myrequests() {
  ...
  // Fetch sent hookups from backend
  const { 
    data, 
    isLoading, 
    error, 
    refetch,
    isFetching 
  } = useQuery<SentHookupsResponse>({
    queryKey: ['my-sent-hookups'],
    queryFn: async () => {
      const response = await fetch(`${apiUrl}/hookup/my-sent-hookups/`, {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch sent hookups')
      }
  ...
  const sentHookups = data?.sent || []
  
  // Sort: unread first, then by created_at (newest first)
  const sortedHookups = [...sentHookups].sort((a, b) => {
  ...
  const unreadCount = sentHookups.filter(h => !h.is_read_by_current_user).length
  ...
  if (isLoading) {
    return (
      <div className="myrequests-loading-container">
        <Spinner 
          size="large" 
          color="#8B6914" 
    ...
    return (
      <div className="myrequests-error-container">
        <div className="error-card">
          ...
          <p>There was an error loading your sent hookup requests. Please try again.</p>
  ...
  return (
    <div className="overall-my-requests">
      {!showDetail ? (
        <>
          <div className="requests-header">
            <div className="header-title-section">
              <h2 className="requests-title">My Sent Requests</h2>
              {unreadCount > 0 && (
            ...
            {sortedHookups.length === 0 ? (
              <div className="no-requests-container">
                <div className="no-requests-content">
                  ...
                  <p>When you send a romantic hookup request, it will appear here.</p>
                </div>
              </div>
            ) : (
              sortedHookups.map((hookup) => {
                ...
                return (
                  <Notificationpreview
                    key={hookup.id}
                    hookupId={hookup.id}
                    name={hookup.receiver_name}
                    status={hookup.status}
                    timeAgo={getTimeAgo(hookup.created_at)}
                    imgUrl={hookup.receiver_profile_img}
                    isUnread={isUnread}
