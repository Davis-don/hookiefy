...
async function createServiceSeeker(
  payload: SignupPayload
): Promise<SignupResponse> {
  const response = await fetch(
    `${API_BASE}/account/signup/service-seeker/`,
    {
      method: 'POST',
...
async function googleServiceSeeker(
  idToken: string
): Promise<SignupResponse> {
  const response = await fetch(
    `${API_BASE}/account/google/service-seeker/`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function Serviceseeker() {
  ...
  const googleSignupMutation = useMutation({
    mutationFn: googleServiceSeeker,
    onSuccess: (result) => {
  ...
  const localSignupMutation = useMutation({
    mutationFn: createServiceSeeker,
    onSuccess: (result) => {
      const name = result.user?.first_name || 'there';
      if (result.access) localStorage.setItem('access_token', result.access);
      if (result.refresh) localStorage.setItem('refresh_token', result.refresh);
  ...
  return (
    <div className="overall-service-seeker-container">
      <div className="service-seeker-form-container">
